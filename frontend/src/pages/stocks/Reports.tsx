...
export function Reports() {
  ...
  async function loadReports() {
    const data = await fetchReports(mode);
    setReports(data);
  }
  ...
  async function handleTelegram() {
    if (mode === 'real' && !window.confirm('Send the latest REAL mode daily report to Telegram?')) {
      return;
    }
